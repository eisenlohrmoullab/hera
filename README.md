...
Per-participant cyclic GAMs with quarter-coverage gating (>= 2 observations per cycle quarter required). Generates multi-page PDF reports with significance testing, range benchmarking (>= 1.5 range, equivalent to C-PASS 30% change criterion), overlay plots, and individual trajectory visualization. Y-axis defaults to 1-4.5 with dynamic expansion to 5 or 6 when data exceed 4.5 (expanded region shaded pink).
